...
private $izq=19;
    ...
    function datosHorizontal($datos, $nro)
    {
        ...
        $this->SetFont('');
        $this->Cell(20, 10, utf8_decode($datos['num_documento']), 0, 0, 'L');
        $this->SetFont('Arial', 'B', 10);
        $this->Cell(20, 10, 'ESTADO:', 0, 0, 'L');
        $this->SetFont('');
        $this->Cell(20, 10, utf8_decode(($datos['estado']==1?'Activo':'Inactivo')), 0, 1, 'L');
        $this->Ln(-4);
        $this->Cell(10);
        $this->SetFont('Arial', 'B', 10);
    ...
    function getCabecera($idventa)
    {
        /*$sql = "SELECT va.id  
        ,va.estado 
        ,num_extracto
        ,va.fecha_vencimiento 
        ,va.fecha_inicio
        ,va.fecha_cierre
        ,va.fecha_emision 
        ,va.idcliente 
        ...
        ,l.anhociclo anho
        FROM `venta_agua` va 
        inner join lectura l on l.id = va.id_lectura 
        ...
        inner join persona p on p.idpersona = c.id_persona 
        left join zona z on z.id = c.id_zona 
        WHERE va.id = '$idventa' ORDER BY c.id_zona";
    ...
    function getDatosCliente($nrousuario)
    {
        ...
        inner join hidrometro h on h.idcliente = c.id 
        inner join venta_agua va on va.idcliente = c.id
        inner join zona z on z.id = c.id_zona
        where c.id = '$nrousuario'order by nombre asc";
        return ejecutarConsultaSimpleFila($sql);
    }
/*
    function getDatosCliente($nrousuario)
...
*/
/*
    function getNroCliente($mes,$anho)
    {
        $sql = "SELECT DISTINCT  va.`id` 
        FROM `venta_agua` va 
        inner join lectura l on l.id = va.idlectura
        inner join cliente c on c.id = va.idcliente        
        WHERE va.`estado` = 1 and l.mesciclo = '$mes' and l.anhociclo = '$anho'
        ORDER BY c.id_zona,c.orden ASC";
        return ejecutarConsulta($sql);
    }*/

    function getIdVenta($mes,$anho)
    {
        $sql = "SELECT DISTINCT  va.`id` 
        FROM `venta_agua` va 
        inner join lectura l on l.id = va.idlectura
        inner join cliente c on c.id = va.idcliente        
        WHERE  c.estado = 1 and l.mesciclo = '$mes' and l.anhociclo = '$anho'
        ORDER BY c.id ASC";
        return ejecutarConsulta($sql);
    }

    function getIdVentaPersonal($id,$mes,$anho)
    {
        $sql = "SELECT DISTINCT  va.`id` 
        FROM `venta_agua` va 
        inner join lectura l on l.id = va.idlectura
        inner join cliente c on c.id = va.idcliente        
        WHERE  c.estado = 1 and va.idcliente = '$id' and l.mesciclo = '$mes' and l.anhociclo = '$anho'
        ORDER BY c.id ASC";
        return ejecutarConsultaSimpleFila($sql);
    }
