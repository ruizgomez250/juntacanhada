var tabla,ultimalectura,ultimafecha,ind,modif;
var detalles=0;
var cont=-1;
...
function agregarDetalle(id,codigo,cuenta){
		cont++;
		var fila='<tr class="filas" id="fila'+cont+'">'+
        '<td><button type="button" class="btn btn-danger" onclick="eliminarDetalle('+cont+')">X</button></td>'+
        '<td><input type="hidden" name="idcuenta[]" value="'+id+'">'+codigo+'</td>'+
        '<td>'+cuenta+' </td>'+
        '<td><input type="number" name="debe[]" onchange="calcularTotales()" id="cantidad[]" value="0"></td>'+
        '<td><input type="number" name="haber[]" onchange="calcularTotales()" id="precio_compra[]" value="0"></td>'+
		'</tr>';
...
function calcularTotales(){
	...
	for (var i = 0; i < contaux; i++) {
		totalDebe =totalDebe+ (document.getElementsByName("debe[]")[i].value*1);
		totalHaber =totalHaber+ (document.getElementsByName("haber[]")[i].value*1);
		
	}
	document.getElementById("total_debe").innerHTML="Gs." +totalDebe;
	document.getElementById("total_haber").innerHTML="Gs." +totalHaber;
...
function listar(){
	/*var table = $('#tbllistado').DataTable({
        ajax: '../ajax/asientosContables.php?op=listar',
        columns: [
            {
                className: 'dt-control',
                orderable: false,
    ...
    $('#tbllistado tbody').on('click', 'td.dt-control', function () {
        ...
        if (row.child.isShown()) {
			...
			tabla=$('#tbllistado').dataTable({
				...
				"ajax":
				{
					url:'../ajax/asientosContables.php?op=listar',
					type: "get",
					dataType : "json",
					error:function(e){
						console.log(e.responseText);
...
function guardaryeditar(e){
     ...
     $.ajax({
     	url: "../ajax/asientosContables.php?op=guardar",
     	type: "POST",
     	data: formData,
     	contentType: false,
     	processData: false,
...
function guardar(id){
	$('#myModal').modal('hide');
	$.post("../ajax/asientosContables.php?op=modificar",{idOpcion : modif,idCuenta : id},
		function(data,status)
		{
			listar();
			bootbox.alert(data);
...
function listarArticulos(){
	tabla=$('#tblarticulos').dataTable({
		...
		"ajax":
		{
			url:'../ajax/planCuentas.php?op=listarCuentasDetalle',
			type: "get",
			dataType : "json",
			error:function(e){
				console.log(e.responseText);
