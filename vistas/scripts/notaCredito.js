...
function traerPago(id,numdoc,nombre,i,idpago){
	ind=i;
	document.getElementById("idfac").value=idpago;
	document.getElementById("ndoc").innerHTML=numdoc;
	document.getElementById("codus").innerHTML=id;
	document.getElementById("nombMod").innerHTML=nombre;
	$.post("../ajax/caja.php?op=pago",{idpago : idpago},
		function(data,status)
		{
			if(data == 0){
            document.getElementById("tablaModBody").innerHTML='<tr><td></td><td></td><td></td><td></td></tr>';
...
function listar(){
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":
		{
			url:'../ajax/caja.php?op=listarPagado',
			type: "get",
			dataType : "json",
			error:function(e){
				console.log(e.responseText);
...
function guardaryeditar(e){
     ...
     $.ajax({
     	url: "../ajax/caja.php?op=guardarNotaCred",
     	type: "POST",
     	data: formData,
     	contentType: false,
     	processData: false,
