...
//funcion listar
function listar(){
	codUs=document.getElementById('fech').value;
	//alert(codUs);
	//if(fecha.length >0){
			tabla=$('#tbllistado').dataTable({
				...
				"ajax":
				{
					url:'../ajax/borrarDeudaAnt.php?op=listarDeuda&fecha='+codUs,
					type: "get",
					dataType : "json",
					error:function(e){
						console.log(e.responseText);
...
function eliminar(idlectura){
	bootbox.confirm("¿Esta seguro de eliminar este dato?", function(result){
		if (result) {
			...
			$.post("../ajax/borrarDeudaAnt.php?op=eliminar", {idlectura : idlectura }, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
