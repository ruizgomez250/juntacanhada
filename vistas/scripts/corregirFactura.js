...
function listar(){
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":
		{
			url:'../ajax/corregirFactura.php?op=listar',
			type: "get",
			dataType : "json",
			error:function(e){
...
function guardar(ind){
     /*e.preventDefault();//no se activara la accion predeterminada 
     //$("#btnGuardar").prop("disabled",true);
     var formData=new FormData($("#formulario")[0]);
     //alert('llego');*/
     num=document.getElementById("numfac"+ind).value;
     

     $.post("../ajax/corregirFactura.php?op=guardaryeditar",{id : ind,numfac : num},
...
function anular(idimpuesto){
	bootbox.confirm("¿Esta seguro de borrar este dato?", function(result){
		if (result) {
