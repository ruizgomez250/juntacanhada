...
public function insertar($codCliente,$idfactura,$diferencia,$monto,$fecha,$idusuario){
	if($idfactura == 0){
		$sql="SELECT  max(id) id  FROM costos_cliente WHERE pagos_realizados < cantidad_pago and id_cliente=".$codCliente.";";
	...
	while ($reg=$rspta->fetch_row()) { 
	    ...
	    if(!$reg1){
		    ...
		    if($diferencia < 1){
		        ...
		        $sql="INSERT INTO asiento_detalle (id_asiento,id_cuenta,monto,tipo) VALUES ('$idAsiento','$idcuenta','$precio','1');";
		        ejecutarConsulta($sql);
		        $monto=$monto-$precio;
	    ...
	    $sql="SELECT idcuentacontable,precio_venta FROM venta_detalle WHERE concepto like 'Consumo de Agua Potable' AND idventa=$idventa;";
	    $reg1=ejecutarConsultaSimpleFila($sql);
	    if(!$reg1){
	    }else{  
		    $idcuenta=$reg1['idcuentacontable'];
	    ...
	    $sql="SELECT idcuentacontable,precio_venta FROM venta_detalle WHERE not concepto like 'Consumo de Agua Potable' AND not concepto like 'ERSSAN' AND idventa=$idventa;";
	    $reg1=ejecutarConsultaSimpleFila($sql);
	    $idcuenta=$reg1['idcuentacontable'];
	    $precio=$reg1['precio_venta'];
	    if($idcuenta < 1){
	    ...
	    while ($reg1=$rspta1->fetch_row()) {
	      ...
	      $idcuenta=$reg1[0];
	      $sql="INSERT INTO asiento_detalle (id_asiento,id_cuenta,monto,tipo) VALUES ('$idAsiento','$idcuenta','$montoAct','2');";
...
public function insertarNotaCred($timbrado,$numero,$idfac,$fecha,$descuento,$idusuario){
	...
	if($mont >= $descuento){
		...
		$asiento=ejecutarConsulta($sql);
		$sql="UPDATE asiento_detalle SET monto=monto-'$descuento' WHERE id_asiento='$idAsiento' AND monto='$mont' AND tipo='1';";
		$asiento=ejecutarConsulta($sql);

		$sql="INSERT INTO nota_credito (idpago,idusuario,numero_doc,timbrado,descuento,fecha) VALUES ('$idfac','$idusuario','$numero','$timbrado','$descuento','$fecha');";
		//echo($sql);
		ejecutarConsulta($sql);
...
public function traerTimbrado($numFac){
	$sql="SELECT  timbrado  FROM timbrado WHERE '$numFac' BETWEEN desde AND hasta;";
	return ejecutarConsultaSimpleFila($sql);
}
...
public function pago($idpago){
	...
	$sql="SELECT monto FROM pago_factura  WHERE idpago=$idpago;";
	//echo($sql);
	return ejecutarConsultaSimpleFila($sql);
}
//listar registros
...
public function listarFacturas($fecha,$fechaDesde){
	$sql="SELECT f.numfactura,v.idcliente,p.nombre,p.apellido,p.num_documento,f.fecha,f.monto,v.id FROM pago_factura f INNER JOIN venta_agua v on v.id = f.idventaagua INNER JOIN cliente c on c.id=v.idcliente INNER JOIN persona p on p.idpersona=c.id_persona  WHERE f.fecha  BETWEEN '$fechaDesde' AND '$fecha' ORDER BY f.idventaagua;";
	return ejecutarConsulta($sql);

	//echo($sql);
}
public function listarErssan($idventa){
	$sql="SELECT precio_venta FROM venta_detalle WHERE idventa=$idventa AND concepto like 'ERSSAN';";
	return ejecutarConsultaSimpleFila($sql);
