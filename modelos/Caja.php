	...
	while ($reg=$rspta->fetch_row()) { 
	    ...
	    if(!$reg1){
		    ...
		    if($diferencia < 1){
		        ...
		        $sql="INSERT INTO asiento_detalle (id_asiento,id_cuenta,monto,tipo) VALUES ('$idAsiento','$idcuenta','$precio','2');";
		        ejecutarConsulta($sql);
		        $monto=$monto-$precio;
...
public function insertarNotaCred($timbrado,$numero,$idfac,$fecha,$descuento,$idusuario){
	...
	if($mont >= $descuento){*/
		...
		$sql="INSERT INTO nota_credito (idpago,idusuario,numero_doc,timbrado,descuento,fecha,idasiento) VALUES ('$idfac','$idusuario','$numero','$timbrado','$descuento','$fecha','$idAsiento');";
		//echo($sql);
		return ejecutarConsulta($sql);
		/* return $sql;
