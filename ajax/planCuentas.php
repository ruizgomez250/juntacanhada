<?php 
require_once "../modelos/PlanCuentas.php";
if (strlen(session_id())<1) 
	session_start();

$ingreso=new PlanCuentas();
...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>'<button class="btn btn-success" onclick="agregarDetalle('.$reg->id.',\''.$reg->codigo.'\',\''.$reg->cuenta.'\')" ><span class="fa fa-check"></span></button>',
            "1"=>$reg->codigo,
            "2"=>$reg->cuenta
