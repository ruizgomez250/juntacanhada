<?php 
require_once "../modelos/AnularFactura.php";
//include('../config/Conexion1.php');
$corregirF=new AnularFactura();
...
switch ($_GET["op"]) {
	...
	case 'anular':
		...
		$rspta = $corregirF->anular($idFactura);
		echo $rspta ? "Factura Anulada" : "No se pudo anular la Factura";
		//echo($sql);
		break;
    ...
    $numFac=$_GET['nrofac'];
		...
		while ($reg=$rspta->fetch_object()) {
            ...
            "4"=>'<button class="btn btn-danger btn-xs" onclick="anular('.$id.')"><i class="fa fa-close"></i></button>'
              );
		}
		$results=array(
