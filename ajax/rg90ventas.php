<?php 
require_once "../modelos/ReporteParaErssan.php";

$persona=new RG90Ventas();
		  ...
		  case 'listar':
					...
					while ($reg=$rspta->fetch_object()) {	
						...
						if(isset($rspta1['id'])){
							...
							if(isset($rspta1['id'])){
								$idFac=$rspta1['id'];
								$rspta1=$persona->verificarSaldo($idFac);
								$monto=$rspta1['diferencia'];
								$rspta1=$persona->compararDif($idFac);
								if(isset($rspta1['tot'])){
									if($rspta1['tot'] > $monto){
					...
					$cont=0;
					$rspta=$persona->listarFacturas($fecha,$fechaDesde);
					$data=Array();
					$idVenta=-1;
					$totalAbs=0;
					$totalErssan=0;
					...
					while ($reg=$rspta->fetch_object()) {
						...
						$total=0;
						if($idVenta != $reg->id){
							$rspta1=$persona->listarErssan($reg->id);
							if(isset($rspta1)){
								$erssan=$rspta1['precio_venta'];
							}
							
							$consumo=$reg->monto - $erssan;
							$idVenta=$reg->id;
						}else{
							$consumo=$reg->monto;
						}
						$timb=$persona->traerTimbrado($reg->numfactura);
						$timbrado=0;
						if(isset($timb)){
							$timbrado=$timb["timbrado"];
						}
						$total=$consumo+$erssan;
						$totalAbs=$totalAbs+$total;
						$totalErssan=$totalErssan+$erssan;
					...
					$data[]=array(
			            ...
			            "4"=>'',
									"5"=>'',
									"6"=>'',
			            "7"=>'<strong>'.$totalConsumo.'</strong>',
			            "8"=>'<strong>'.$totalErssan.'</strong>',
									"9"=>'<strong>'.$totalAbs.'</strong>');
					$results=array(
			             "sEcho"=>1,//info para datatables
			             "iTotalRecords"=>count($data),//enviamos el total de registros al datatable
			             "iTotalDisplayRecords"=>count($data),//enviamos el total de registros a visualizar
