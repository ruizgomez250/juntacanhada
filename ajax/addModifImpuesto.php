...
switch ($_GET["op"]) {
	case 'guardaryeditar':
			...
			if (strlen($idimpuesto) < 1) {
				...
				$sql = "INSERT INTO impuesto (estado,descripcion,porcentaje,id_cuenta) VALUES ($estado,'$descripcion',$porcentaje,$idcuenta);";
		    ...
		    $estado=$_POST['estado'];
				...
				$descripcion=$_POST['descripcion'];
				$idcuenta=$_POST['idcuenta'];
				$sql = "UPDATE impuesto set estado=$estado,descripcion='$descripcion',porcentaje=$porcentaje,id_cuenta=$idcuenta WHERE id=$idimpuesto;";
				$rspta = $conexion->query($sql);
				echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar los datos"; 
				//echo($sql);
			}
