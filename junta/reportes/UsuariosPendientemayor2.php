...
/******************** Dar Forma **************************/
	...
	$pdf->Ln();
	$pdf->SetFont('Arial', 'B', 13);
	$pdf->Cell(0, 5,utf8_decode('FACTURAS PENDIENTE DE PAGO A PARTIR DE 3 ORDENES POR USUARIO'), 0, 0, 'C');
	$pdf->Ln();
/********** Fin Dar Forma*********/
/*********** fin Encabezado ********************/
/********* Tabla Cabecera****************/
...
$sql = "SELECT id,fecha_emision,id_cliente,vencimiento FROM factura_encabezado WHERE estado=1 ORDER BY id_cliente asc;";
...
while( $row = pg_fetch_array( $stmt) ) {
	...
	$id_cliente=$row[2];
	$vencimiento=$row[3];
	$sql123 = "SELECT count(id) FROM factura_encabezado WHERE estado=1 and id_cliente=$id_cliente;";
	$resultado123 = pg_query($conexion,$sql123);
	$res123 = pg_fetch_row($resultado123);
	$cont = $res123[0];
	if($cont > 2){
