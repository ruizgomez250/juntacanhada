...
/******************** Dar Forma **************************/
	...
	$pdf->Ln();
	$pdf->SetFont('Arial', 'B', 13);
	$pdf->Cell(0, 5,utf8_decode('FACTURAS PAGADAS ULTIMA FACTURACION'), 0, 0, 'C');
	$pdf->Ln();
/********** Fin Dar Forma*********/
/*********** fin Encabezado ********************/
/********* Tabla Cabecera****************/
$pdf->SetFont('Arial', 'B', 10);
	...
	$pdf->Cell(30, 7, 'SALDO', 1, 0, 'C', true);
	$pdf->Ln();
/***************Fin Tabla Cabecera*************************/
...
if(strlen($fechEmision > 1)){/***********cerrar*********/
	$sql = "SELECT f.id,f.fecha_emision,f.id_cliente,f.vencimiento FROM factura_encabezado f INNER JOIN cliente c on c.id = f.id_cliente WHERE f.estado=2 and f.fecha_emision = '$fechEmision' ORDER BY c.orden;";
	...
	while( $row = pg_fetch_array( $stmt) ) {
		...
		/********** Verifica si tiene saldo*********/
		$montoTot=0;
		$sql15 = "SELECT c.id,c.descripcion,c.monto_total
      FROM costo c INNER JOIN costos_x_cliente cosC on cosC.id_costo=c.id WHERE c.descripcion = 'Saldo Pago Anterior Cod.#s' and cosC.numero_de_pago=0 and cosC.id_usuario=$id_cliente;";
	  $stmt15 = pg_exec($conexion, $sql15 );
	  $bodyTablaPermiso=null;
	  while( $row15 = pg_fetch_array( $stmt15) ) {
	        $montoTot=$row15['monto_total'];
	   }
		/*************Fin verificar***********/
		$pdf->SetFont('Arial', '', 9);
		$pdf->SetFillColor(255, 255, 255);
		...
		$pdf->Cell(30, 7,$vencimiento, 1, 0, 'C', true);
		$pdf->Cell(30, 7,$periodo, 1, 0, 'C', true);
		$pdf->Cell(30, 7,number_format($montoTot, 0, ',', '.'), 1, 0, 'C', true);	
		$pdf->Ln();
