...
$contln=0;
$total=0;
/************ VERIFICA EL PORCENTAJE DE MOROSIDAD******************/
$morosos=0;
$clientesActivos=0;
$sql = "SELECT count(id) FROM factura_encabezado WHERE estado=1 and fecha_emision<'$fecha_emision';";
...
$sql = "SELECT count(id) FROM cliente WHERE estado=1;";
$stmt = pg_exec($conexion, $sql );

while( $row = pg_fetch_array( $stmt) ) {
	
	$clientesActivos=$row[0];
}
$porcentajeMorosidad=($morosos*100)/$clientesActivos;
/************************* FIN VERIFICAR***********************/
/**********ENCABEZAD*********/
$sql = "SELECT f.id,f.fecha_emision,f.id_cliente,f.vencimiento,c.id_medidor FROM factura_encabezado f INNER JOIN cliente c on c.id=f.id_cliente WHERE fecha_emision='$fecha_emision' ORDER BY c.orden;";
$stmt = pg_exec($conexion, $sql );
...
/*********FIN LECTURAS***********/
	...
	$pdf->SetFont('Arial', '', 9);
	//$pdf->Cell(0, 5,utf8_decode('Porcentaje de Morosidad a la fecha de emision desde periodo febrero 2022: '.number_format($porcentajeMorosidad, 0, ',', '.').'%'), 0, 0, 'C');
	//$pdf->Ln();
	$pdf->SetFillColor(255, 255, 255);
	$pdf->SetDrawColor(255, 255, 255);
