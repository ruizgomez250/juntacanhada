...
function cargarTabla1(){
    ...
    $(document).on("click",".ieditar",function(){
                ...
                document.getElementById('medidorModal').value=idEnviado;

                parametro='medidor='+idEnviado;
                cargarTablaModal(parametro);
                $('#modalTomaMedida').modal('show');
                
        })
    $(document).on("click",".borrar",function(){
                idMedida =$(this).attr("name");
                $.ajax({
                url: "../ajax/TomaMedida.php?op=borrarMedida",
                type: "POST",
                data: "id="+idMedida,
   ...
   function cargarTablaModal(parametro){
        $.ajax({
                url: "../ajax/TomaMedida.php?op=BuscarTomaMedidaTabla",
                type: "POST",
                data: parametro,
                success: function(res){
                    var datos = JSON.parse(res);
                    var elmtTable = document.getElementById("tablaMedida");
                            var tableRows = elmtTable.getElementsByTagName("tr");
                            var rowCount = tableRows.length;
                            for (var x=rowCount-1; x>0; x--) {
                                document.getElementById("tablaMedida").deleteRow(x);
                            }
                            for (var y=datos[0]; y>0; y--) {
                                    document.getElementById("tablaMedida").insertRow(1).innerHTML = datos[y];
                            }                   
                }
            });
...
function cargarModalOpcion($dominio,$tablaNombre){
 ...
 function cargarTabla(parametro){
 		$.ajax({
				url: "../php/cliente.php?op=buscarClienteTabla&medida=1",
				type: "POST",
				data: parametro,
				success: function(res){
					var datos = JSON.parse(res);
