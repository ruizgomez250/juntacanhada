...
$('#buscUsuario').click(function() { 
    ordenUsu=1;   
    $('#modalUsuario').modal('show');

})
$('#buscCosto').click(function() {
    ordenUsu=2;
    $('#modalUsuario').modal('show');
})
$(document).on("click",".selecCosto",function(){

    categoriaId =$(this).attr("name");
    parametro='id='+categoriaId;
    cargaDatosCosto(parametro);

})
function cargaDatosCosto(parametro){
    $.ajax({
        url: "../ajax/asignarCostoUsuario.php?op=traeDatosCosto",
        type: "POST",
        data: parametro,
        success: function(res) {
            if(res ==0){
                document.getElementById('codCosto').value='';
                document.getElementById('descCosto').value='';
                document.getElementById('montoTot').value='';
                costoVerif=false;
            }else{
                var datos = JSON.parse(res);
                document.getElementById('codCosto').value=datos[0];
                document.getElementById('descCosto').value=datos[1];
                document.getElementById('montoTot').value=datos[2];
        ...
        success: function(res) {
            if(res ==0){
                ...
                if(ordenUsu == 1){
                    ...
                    var datos = JSON.parse(res);
                    document.getElementById('codCosto').value=datos[0];
                    document.getElementById('montoTot').value=datos[1];
                    document.getElementById('descCosto').value=datos[2];
                    $('#modalUsuario').modal('hide');
                    costoVerif=true;
                }
            }
...
var cantP= document.getElementById('cantPago');
cantP.onchange = function(cantPag) {
    id = document.getElementById('cantPago').value;
    if(id.length > 0){
        pagosVerif=true;
    }else{
        pagosVerif=false;
...
var codCost= document.getElementById('codCosto');
codCost.onchange = function(codC) {
    ordenUsu=2;
    id = document.getElementById('codCosto').value;
    if(id.length > 0){
        parametro='id='+id;
        cargaDatosUsuario(parametro);
    }
