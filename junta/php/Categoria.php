<?php
	...
	switch ($op) {
		case 'guardar':
			...
			$sql = 'SELECT coalesce(MAX(id),0) FROM categoria';
			$resultado = pg_query($conexion,$sql);
			$res = pg_fetch_row($resultado);
			$id = $res[0];
			$id++;
			$sql = "INSERT INTO categoria (id,descripcion,costo_consumo_minimo,litros_minimo,costo_por_litro_sobre_minimo) VALUES ($id,'$descCategoria',$costoConsumoMinimo,$litrosConsumoMinimo,$costoLitrosPorExceso);";
			
			$stmt = pg_query($conexion, $sql );
			header("location: ../vistas/addModifCategoria.php?moadfnmgvlfjhdif=".$id);
		exit();
