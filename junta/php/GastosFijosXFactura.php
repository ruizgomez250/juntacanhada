<?php
	...
	switch ($op) {
		case 'guardar':
			...
			$sql = 'SELECT coalesce(MAX(id),0) FROM costos_fijos_x_factura';
			$resultado = pg_query($conexion,$sql);
			$res = pg_fetch_row($resultado);
			$id = $res[0];
			$id++;
			$sql = "INSERT INTO costos_fijos_x_factura (id,descripcion,monto,porcentaje,es_porcentaje,estado) VALUES ($id,'$descripcion',$costoFijoGuaranies,$porcentajeCosto,$esPorcentaje,$estado);";
		...
		case 'modificar':
			...
			$sql = "UPDATE costos_fijos_x_factura SET id=$id,descripcion='$descripcion',monto=$costoFijoGuaranies,porcentaje=$porcentajeCosto,es_porcentaje=$esPorcentaje,estado=$estado WHERE id=$id;";
			$stmt = pg_query($conexion, $sql );
			header("location: ../vistas/addModifGastosFijosXfactura.php?moadfnmgvlfjhdif=".$id);
		break;
	}
