<?php
	...
	switch ($op) {
		case 'guardar':
			...
			$codigoCosto=$_POST["codCosto"];
			$cantidadPago=$_POST["cantPago"];
			

			$sql = 'SELECT coalesce(MAX(id),0) FROM costos_x_cliente';
			$resultado = pg_query($conexion,$sql);
			$res = pg_fetch_row($resultado);
			$id = $res[0];
			$id++;
			$sql = "INSERT INTO costos_x_cliente (id,cantidad_pagos,estado,numero_de_pago,id_usuario,id_costo) VALUES ($id,$cantidadPago,$estado,$pagado,$codigoUsuario,$codigoCosto);";
			
			$stmt = pg_query($conexion, $sql );
			header("location: ../vistas/asignarCostoUsuario.php?moadfnmgvlfjhdif=".$id);
		exit();
		...
		case 'modificar':
			...
			$codigoCosto=$_POST["codCosto"];
			$cantidadPago=$_POST["cantPago"];
			$sql = "UPDATE costos_x_cliente set cantidad_pagos=$cantidadPago,id_usuario=$codigoUsuario,id_costo=$codigoCosto where id=$id;";
			
			$stmt = pg_query($conexion, $sql );

			header("location: ../vistas/asignarCostoUsuario.php?moadfnmgvlfjhdif=".$id);
			/*
			$descripcion=$_POST["descCostoFijo"];
			$costoFijoGuaranies=0;
			$esPorcentaje=0;
			if(isset($_POST["costoGuaranies"])){
				$costoFijoGuaranies=$_POST["costoGuaranies"];
			}
			$porcentajeCosto=0;
			if(isset($_POST["porcentajeCosto"])){
				$esPorcentaje=1;
				$porcentajeCosto=$_POST["porcentajeCosto"];
			}

			$sql = "UPDATE costos_fijos_x_factura SET id=$id,descripcion='$descripcion',monto=$costoFijoGuaranies,porcentaje=$porcentajeCosto,es_porcentaje=$esPorcentaje WHERE id=$id;";
			
			$stmt = pg_query($conexion, $sql );
			header("location: ../vistas/addModifGastosFijosXfactura.php?moadfnmgvlfjhdif=".$id);*/
		break;
	}
?>